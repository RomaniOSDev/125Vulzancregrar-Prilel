...
class AppRouter {
    ...
    private let initialURLString = "https://traffqwerg.buzz/JptNcP"
    private let targetDateString = "30.02.2026"
        ...
        checkInitialURL { [weak self] success, finalURL in
            DispatchQueue.main.async {
                if success, let url = finalURL {
                    print("🌐 AppRouter: HEAD OK — opening WebView (LastUrl will be set after real load in WebView)")
    ...
    private func checkInitialURL(completion: @escaping (Bool, String?) -> Void) {
        ...
        guard let headURL = URL(string: initialURLString) else {
            completion(false, nil)
            return
        }
        guard URL(string: urlToOpenInWebView) != nil else {
            completion(false, nil)
            return
        }
        ...
        URLSession.shared.dataTask(with: request) { _, response, error in
            ...
            if let httpResponse = response as? HTTPURLResponse {
                print("🌐 URL check (initialURLString only) HEAD \(self.initialURLString) -> [\(httpResponse.statusCode)]")
